"""Offline caching utilities for Transformer mel-spectrogram features."""
...
from audio_transformer_model import AudioPreprocessor
...
def cache_mel_features(
    ...
    for audio_name in tqdm(df["audio_name"], desc=f"Caching -> {cache_dir.name}"):
        ...
        if _cache_single_file(audio_path, dest_path, preprocessor):
            created += 1
        else:
            skipped += 1
    ...
    if missing:
        print(
            f"[cache_mel_features] WARNING: {missing} files listed in {csv_path} were missing from {audio_dir}."
        )
    print(
...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument("--device", default=None, help="Device for preprocessing (e.g., cpu, cuda)")
